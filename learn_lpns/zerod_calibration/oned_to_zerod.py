...
def extract_observations_from_1d(
    ...
    {v["vessel_name"]: i for i, v in enumerate(vessels)}
    ...
    if time_step_size is not None:
        ...
        print(
            f"  Calculated dt for derivatives: {dt:.6f} s "
            f"(time_step_size * increment = {time_step_size:.6f} * {timestep_increment})"
        )
    ...
    def extract_at_point(point_idx, times, dt, deriv_method="backward", verbose=False):
        ...
        pressure_data = np.array([centerline_data[pt][point_idx] for pt in pressure_timesteps])
        flow_data = np.array([centerline_data[ft][point_idx] for ft in flow_timesteps])
    ...
    for vessel in vessels:
        if "boundary_conditions" in vessel:
            ...
            if bc_outlet:
                ...
                int(vessel["vessel_name"].split("_")[0].replace("branch", ""))
                ...
                if outlet_point_idx is not None:
                    p_ref, p_der, f_ref, f_der = extract_at_point(
                        outlet_point_idx, times, dt, derivative_method, verbose=False
                    )
                    if p_ref is not None:
                    ...
                    print(
                        f"  Warning: Could not find outlet point for {vessel['vessel_name']}:{bc_outlet}, using zeros"
                    )
    ...
    for junc in junctions:
        ...
        for vessel_id in inlet_vessel_ids:
            if vessel_id < len(vessels):
                ...
                int(vessel_name.split("_")[0].replace("branch", ""))
...
def extract_observations_from_1d_with_node_ids(
    ...
    def find_point_from_node_id(vessel, is_inlet=True):
        ...
        Raises if node IDs missing or not in centerline.
        """
    ...
    if cascade_split_connector_flows:
        from learn_lpns.zerod_calibration.bifurcation_splitting import apply_split_connector_flow_cascade
...
def identify_junctions_and_bcs(centerline_data, vessels):
    ...
    """
    branch_id = centerline_data.get("BranchId", None)
    centerline_data["Points"]
    centerline_data.get("Cells", [])
    ...
    for i, vessel in enumerate(vessels):
        if i not in all_outlet_vessels:
            ...
            boundary_conditions.append(
                {"bc_name": bc_name, "bc_type": "RESISTANCE", "bc_values": {"Pd": 0.0, "R": 1.0}}
            )
...
def verify_inlet_flow_matches_bc(input_data, results, inlet_vessel_name="branch0_seg0"):
    ...
    if HAS_SCIPY_INTERP:
        ...
        try:
            interp_func = interp1d(bc_times, bc_flows, kind="linear", bounds_error=False, fill_value="extrapolate")
