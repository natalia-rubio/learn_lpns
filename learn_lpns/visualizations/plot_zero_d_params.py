...
Usage:
  python learn_lpns/visualizations/plot_zero_d_params.py --set_name VMR --geo_name 0063_1001 \\
    --modalities NORMAL_JUNCTION BloodVesselJunction --verbose
...
def find_modality_jsons(data_dir):
    ...
    bif_candidates = glob.glob(os.path.join(data_dir, "*bifurcations*geometric*.json")) + glob.glob(
        os.path.join(data_dir, "bifurcations_geometric_input.json")
    )
    if bif_candidates:
    ...
    calib_patterns = [
        "*_calibrated_output_*.json",
        "*_calibrated_output*.json",
        "calibrated_output_*.json",
    ]
    calib_files = []
...
def main():
    ...
    parser.add_argument(
        "--data_root",
        default=None,
        help="Root data/zeroD directory (default: ../../data/zeroD relative to script)",
    )
    parser.add_argument(
        "--run_config",
        default="base",
    ...
    parser.add_argument(
        "--output_root",
        default=None,
        help="Output results root (default: results/param_comparison)",
    )
    parser.add_argument(
        "--modalities",
        nargs="*",
        help=(
            "Optional list of modalities to include (e.g., NORMAL_JUNCTION BloodVesselJunction). "
            "If omitted, all detected modalities are used."
        ),
    )
    ...
    out_path = plot_zero_d_parameter_bars(
        modality_jsons, output_dir=out_dir, output_name=out_name, verbose=args.verbose
    )
    if out_path:
