...
from learn_lpns.visualizations.cv_pressure_errors_to_latex import (
    MODALITY_DISPLAY,
    format_modality_display_for_legend,
)
from learn_lpns.visualizations.matplotlib_tex import configure_matplotlib_latex, latex_is_available
...
def plot_location_comparison(
    ...
    geometric_results.get("geometric_0d", {}).get("pressures")
    geometric_results.get("geometric_0d", {}).get("flows")

    # Extract 3D observations from calibration input
    times_3d_norm, pressures_3d, flows_3d = extract_data_from_calibration_input(calibration_input_path, location)
        ...
        if geo_data["times"] is not None and len(geo_data["times"]) > 0:
            ...
            if np.any(zoom_mask):
                geometric_results_zoom[geo_key] = {
                    ...
                    "pressures": np.array(geo_data["pressures"])[zoom_mask]
                    if geo_data["pressures"] is not None
                    else None,
    ...
    def plot_pressure_data(
        ...
        pressures_3d_data,
        geometric_data_dict,
        calibrated_data_dict,
        set_ylim_from_3d=False,
    ):
        if pressures_3d_data is not None:
            ax.plot(
                times_data,
                pressures_3d_data,
                color=style_3d["color"],
                linestyle=style_3d["linestyle"],
                linewidth=style_3d["linewidth"],
                label=style_3d["label"],
    ...
    plot_pressure_data(
        ...
        times_3d_top,
        pressures_3d_top,
        geometric_results_top,
        calibrated_results_top,
        set_ylim_from_3d=False,
    )
...
def main():
    parser = argparse.ArgumentParser(description="Plot pressure and flow comparison at any location in the network")
    ...
    parser.add_argument(
        "--location",
        help='Specific location to plot (e.g., "INFLOW:branch0_seg0" or "branch0_seg0:J0")',
    )
    ...
    parser.add_argument(
        "--output_dir",
        default="results/location_comparison",
        help="Output directory for plots (run-config subfolder is appended)",
    )
    parser.add_argument("--data_dir", default="data/zeroD", help="Data directory for input files")
    parser.add_argument(
        "--geometry_variant",
        default=None,
        help=(
            "Geometry variant (e.g. bifurcations_EL). When set, also load NN modalities "
            "from {variant}_NN_*_results.csv if present."
        ),
    )
    parser.add_argument(
        "--time_period",
    ...
    parser.add_argument("--zoom_start", type=int, default=None, help="Start index for zoom window")
    parser.add_argument("--zoom_end", type=int, default=None, help="End index for zoom window")
    ...
    if args.location:
        ...
        if len(vessel_order) > 10:
            ...
            if args.verbose:
                print(
                    f"  ⚠ More than 10 unique vessels ({len(vessel_order)}). "
                    f"Limiting to first 10 vessels for plotting: {selected_vessels}"
                )
