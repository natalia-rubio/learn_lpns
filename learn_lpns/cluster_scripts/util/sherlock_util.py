...
from util.junction_proc import load_centerline_data
...
def set_up_sim_directories(anatomy, set_type, geo_name, flow_name, num_procs):
    ...
    if os.path.exists(
        f"/scratch/users/nrubio/synthetic_junctions/{anatomy}/{set_type}/{geo_name}/{flow_name}/{num_procs}-procs_case"
    ):
        os.system(
            f"rm -r /scratch/users/nrubio/synthetic_junctions/{anatomy}/{set_type}/{geo_name}/{flow_name}/{num_procs}-procs_case"
        )
...
def check_for_centerline(anatomy, set_type, geo_name):
    centerline_dir = (
        f"/scratch/users/nrubio/synthetic_junctions/{anatomy}/{set_type}/{geo_name}/centerlines_simVascular.vtp"
    )
    pt_id, num_pts, branch_id, junction_id, area, angle1, angle2, angle3, path, points = load_centerline_data(
    ...
    try:
        ...
        print(
            f"Couldn't process centerline at /scratch/users/nrubio/synthetic_junctions/{anatomy}/{set_type}/{geo_name}/centerlines_simVascular.vtp"
        )
...
def get_cap_info(anatomy, set_type, geo_name, correct_cap_numbers=2):
    inlet_cap_number = int(
        np.load(f"/scratch/users/nrubio/synthetic_junctions/{anatomy}/{set_type}/{geo_name}/max_area_cap.npy")[0]
    )
    cap_numbers = get_cap_numbers(
        f"/scratch/users/nrubio/synthetic_junctions/{anatomy}/{set_type}/{geo_name}/mesh-complete/mesh-surfaces/"
    )
    # print(cap_numbers)
...
def load_params_dict(anatomy, set_type, geo_name):
    try:
        params_dict = load_dict(
            f"/scratch/users/nrubio/synthetic_junctions/{anatomy}/{set_type}/{geo_name}/junction_params_dict"
        )
    except Exception:
        print(
            f"Couldn't load parameter dictionary: /scratch/users/nrubio/synthetic_junctions/{anatomy}/{set_type}/{geo_name}"
        )
