...
Usage (from repo root; use the **svVasc_clean** conda env for VTK, meshio, numpy, tqdm):
  ...
  PYTHONPATH=. python3 learn_lpns/cluster_scripts/project_tst_cohort_centerline_python.py --dry_run
  PYTHONPATH=. python3 learn_lpns/cluster_scripts/project_tst_cohort_centerline_python.py --case TST-5

One-liner without activating (sets ``PYTHONPATH`` in the subprocess):

  conda run -n svVasc_clean --no-capture-output bash -lc \\
    'cd /path/to/learn_lpns && export PYTHONPATH=. && python3 learn_lpns/cluster_scripts/project_tst_cohort_centerline_python.py --case TST-5'
"""
...
# ``cluster_scripts/util/`` package — that ``util`` must appear before repo ``learn_lpns``.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "`PYTHONPATH=. python3 learn_lpns/cluster_scripts/project_tst_cohort_centerline_python.py` "
            "from the repo root."
        ),
