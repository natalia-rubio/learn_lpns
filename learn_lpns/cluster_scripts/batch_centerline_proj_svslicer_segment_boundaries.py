...
def combine_vtu_files(time_files, output_vtu_path):
    ...
    try:
        ...
        for i in range(num_point_arrays):
            ...
            if not (
                array_name and (array_name.lower().startswith("pressure") or array_name.lower().startswith("velocity"))
            ):
        ...
        if output_created:
            ...
            if result.stdout:
                ...
                for line in lines[-10:]:  # Last 10 lines
                    if any(
                        keyword in line.lower() for keyword in ["completed", "writing", "slice extraction", "per slice"]
                    ):
                        print(f"  {line.strip()}")
...
def project_results_python_fallback(
    geo_dir,
    sim_dir,
    centerline_path,
    num_procs,
    ...
    try:
        ...
        print(
            f"  Filtering points: {num_points_to_process}/{num_total_points} points within {num_points_near_boundary} points of segment boundaries"
        )
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            # Submit all tasks
            futures = {executor.submit(process_point_wrapper, data): data[0] for data in point_data_with_vtu}
...
def process_geometry(
    ...
    try:
        ...
        return project_results_python_fallback(
            geo_dir,
            sim_dir,
            centerline_path,
            num_procs,
            ...
            time_files,
            num_workers=None,
            num_points_near_boundary=num_points_near_boundary,
        )
