#!/usr/bin/env bash
# One-shot environment for k-fold cross-validation (learn-lpns-cv).
#
# Does:
#   1. Clone/update learn_lpns + svZeroDPlus (sibling checkout under workspace)
#   2. cmake build svzerodsolver and svzerodcalibrator → SVZEROD_INSTALL_DIR
#   3. Create .venv; pip install JAX + editable learn_lpns[dev] (from pyproject.toml)
#   4. Verify bundled sample data under data/
#   5. Regenerate scripts/cv_env.sh (source before running CV)
#
# Usage (from repo root or anywhere):
#   ./scripts/setup_cross_validation.sh
#   source scripts/cv_env.sh
#
# Does not run pre-commit install. See scripts/README.md for all setup scripts.
...
Steps:
  ...
  3. Create .venv and pip install JAX + editable learn_lpns[dev] (unless --skip-python)
