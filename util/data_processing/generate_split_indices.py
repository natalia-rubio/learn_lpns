...
def generate_split_indices(num_pts: int, percent_train: float, seed: int = 0):
    ...
    if num_pts <= 1:
        raise ValueError(f"num_pts must be > 1, got {num_pts}")

    rng = np.random.default_rng(seed)
    indices = rng.permutation(num_pts)
    n_train = int(percent_train * num_pts)
    if n_train <= 0 or n_train >= num_pts:
...
def main():
    ...
    if "input_o1" not in data_dict:
        raise ValueError(f"Expected 'input_o1' in data_dict at {jax_arrays_path}")
    num_pts = int(np.asarray(data_dict["input_o1"]).shape[0])
    ...
    out_dir = os.path.join(args.data_root, "split_indices", args.set_name, args.set_type)
