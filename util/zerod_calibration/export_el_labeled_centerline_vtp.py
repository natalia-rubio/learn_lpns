...
paths under ``data/`` (same layout as ``plot_vessel_boundaries_wrapper``), or pass
explicit ``--geometric-input``, ``--centerline``, and ``--output``.
...
def find_centerline_file(
    ...
    """
    Find centerline VTP (same search order as plot_vessel_boundaries_wrapper).
    """
    possible_paths = [
...
Path modes:
  (1) Provide --set-name and --geo-name (and optionally --run-config): paths are
      inferred under --data-dir (same layout as plot_vessel_boundaries_wrapper).
  (2) Provide --geometric-input, --centerline, and --output explicitly.

Examples:
  python -m util.zerod_calibration.export_el_labeled_centerline_vtp \\
