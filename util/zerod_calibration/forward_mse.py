...
def _aggregate_modality_mse(
    ...
    return {
        ...
        "overall_mse": np.mean(total_mse_pressure + total_mse_flow)
        ...
        "overall_max_error": np.mean(total_max_pressure + total_max_flow)
        if (total_max_pressure or total_max_flow)
        else np.nan,
...
def _modality_column_widths(modalities, mse_results):
    ...
    return [(mod, modality_table_header(mod), max(num_w, len(modality_table_header(mod)))) for mod in active]
...
def _write_mse_comparison_csv(output_csv_path, modalities, mse_results, all_obs_keys, zoom_start_idx, zoom_end_idx):
    modalities = sort_modalities(modalities)
...
def _compute_modality_mse(
    csv_path, obs_3d, zoom_start_idx, zoom_end_idx, zoom_window_calculated, max_3d_length, verbose
):
    ...
    for obs_key, obs_values_3d in obs_3d.items():
        ...
        if verbose:
            print(f"      ✓ Processed {obs_key}: MSE={metrics['mse']:.3E}, max_err={metrics['max_error']:.3E}")
    ...
    return (
        ...
        zoom_start_idx,
        zoom_end_idx,
        zoom_window_calculated,
    )


def calculate_mse_between_3d_and_0d(
