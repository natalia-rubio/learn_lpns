...
def _warn_if_bc_params_changed(bc_name, vessel_name, bc_type, old_values, new_values, *,
...
def _fit_resistance_outlet(vessel_name, bc_name, bc_cfg, observations):
    ...
    if fitted_r < 0:
        print(
            f"  Warning: Negative resistance fitted for {vessel_name}:{bc_name} "
            f"({fitted_r:.4f}), using absolute value"
        )
...
def fit_bcs_from_observations(geometric_input_path, observations, dt=None, fit_pd=False):
    ...
    for vessel in vessels:
        ...
        if bc_type == 'RESISTANCE':
            ...
            if dt is None:
                print(
                    f"  Warning: dt unavailable; skipping RCR fit for "
                    f"{vessel_name}:{bc_name}"
                )
                continue
            result = _fit_rcr_outlet(vessel_name, bc_name, bc_cfg, observations, dt, fit_pd=fit_pd)
        elif bc_type == 'CORONARY':
            print(
                f"  Skipping {vessel_name}:{bc_name} (CORONARY BC; "
                f"using values already in geometric input)"
            )
            continue
        else:
            raise ValueError(
                f"Unsupported outlet bc_type '{bc_type}' for {bc_name} ({vessel_name}); "
                f"expected RESISTANCE or RCR"
            )
    ...
    try:
        ...
        for bc_name, params in fitted_bcs.items():
            ...
            if bc_type == 'RESISTANCE' and bc_cfg.get('bc_type') == 'RESISTANCE':
                ...
                bc_cfg['bc_values']['Pd'] = float(params['Pd'])
                print(
                    f"    {bc_name}: R {old_r:.4f} -> {params['R']:.4f}, "
                    f"Pd {old_pd:.4f} -> {params['Pd']:.4f}"
                )
                updated_count += 1
