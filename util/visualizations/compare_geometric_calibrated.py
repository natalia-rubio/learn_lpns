...
def create_violin_plots(geometric_vessel_values, calibrated_vessel_values,
                        geometric_junction_values, calibrated_junction_values,
                        output_path, geo_name=None, log_scale=None):
    """
    ...
    for idx, element_type in enumerate(element_types):
        ...
        if sns is not None:
            ...
            sns.violinplot(data=df, x='Type', y='Value', ax=ax, 
                          palette=palette, order=['Geometric', 'Calibrated'])
        else:
...
def main():
    ...
    success = create_violin_plots(geometric_vessel_values, calibrated_vessel_values,
                                  geometric_junction_values, calibrated_junction_values,
                                  output_path, geo_name=args.geo_name, log_scale=log_scale)
