...
from util.vtk_functions import read_geo
...
def load_centerline_data(fpath_1d):
    ...
    angle1 = direction[0, :].reshape(
        -1,
    )
    angle2 = direction[1, :].reshape(
        -1,
    )
    angle3 = direction[2, :].reshape(
        -1,
    )
...
def identify_junctions(junction_id, branch_id, pt_id):
    ...
    for i in junction_ids:
        ...
        branch_pts_junc.append(
            max([min(junction_pts) - 1, min(base_branch_pts)])
        )  # find "inlet" of junction (point with smallest Id)
        branch_ids_junc.append(
            branch_id[pt_id == min(junction_pts) - 1][0]
        )  # find the branch to which the inlet belongs
        ...
        for j in branch_ids:
            ...
            if len(shared_pts) != 0 and j not in branch_ids_junc:  # if there is a shared point in the branch
                ...
                branch_pts_junc.append(
                    min([min(branch_pts).astype(int), max(branch_pts)])
                )  # add outlet point Id to outlet point array
...
def identify_junctions_percent_offset(junction_id, branch_id, pt_id, path, points, percent_offset):
    ...
    for i in junction_ids:
        ...
        branch_pts_offset.append(
            max([min(junction_pts) - 100, min(base_branch_pts)])
        )  # find "inlet" of junction (point with smallest Id)
        branch_ids_junc.append(
            branch_id[pt_id == min(junction_pts) - 100][0]
        )  # find the branch to which the inlet belongs
        ...
        for j in branch_ids:
            ...
            if len(shared_pts) != 0 and j not in branch_ids_junc:  # if there is a shared point in the branch
                ...
                branch_pt_offset = min([min(branch_pts).astype(int), max(branch_pts)]) + int(
                    percent_offset * (num_branch_pts - 1)
                )
                ...
                junction_end_pt_id = junction_pts[
                    np.argmin(np.linalg.norm(junction_points - branch_point_junc, axis=1))
                ]
        ...
        # print("asserting")
        assert len(branch_pts_junc) - 1 == len(junction_lengths), (
            "branch_pts should have one more entry than junction_lengths"
        )
        assert len(branch_pts_junc) == len(branch_pts_offset), (
            "branch_pts and branch_pts_offset should have the same length"
        )
        assert len(junction_lengths) == len(branch_lengths), (
            "junction_lengths and branch_lengths should have the same length"
        )
        assert len(junction_lengths) == len(total_lengths), (
            "junction_lengths and total_lengths should have the same length"
        )
        # print("constructing junciton dict")

        junction_dict.update(
            {
                i: {
                    "branch_pts": branch_pts_junc,
...
def identify_junctions_synthetic(junction_id, branch_id, pt_id):
    ...
    for i in junction_ids:
        ...
        branch_ids_junc = []  # branch ids of junction
        branch_pts_junc.append(
            max(min(junction_pts) - 40, min(pt_id[branch_id == 0]))
        )  # find "inlet" of junction (point with smallest Id)
        branch_ids_junc.append(
            branch_id[pt_id == min(junction_pts) - 1][0]
        )  # find the branch to which the inlet belongs
        ...
        for j in branch_ids:
            ...
            if len(shared_pts) != 0 and j not in branch_ids_junc:  # if there is a shared point in the branch
                ...
                branch_ids_junc.append(j.astype(int))  # add outlet branch Id to outlet branch array
                branch_pts_junc.append(
                    min(min(branch_pts + 40), max(branch_pts)).astype(int)
                )  # add outlet point Id to outlet point array
        junction_dict.update({i: branch_pts_junc})
        assert i == 0, "There should only be one junction,"
    return junction_dict, branch_pts_junc
...
def load_vmr_model_data(fpath_1dsol):
    ...
    soln_array["Path"]  # distance along centerline
    ...
    pressure_in_time = np.zeros(
        (0, num_pts)
    )  # initialize pressure_in_time matrix, each column is a mesh point, each row is a timestep
    flow_in_time = np.zeros(
        (0, num_pts)
    )  # initialize flow_in_time matrix, each column is a mesh point, each row is a timestep
    ...
    for key in soln_array.keys():
        if key[0:8] == "pressure":
            ...
            flow_in_time = np.vstack((flow_in_time, soln_array[key]))  # add timestep column to flow_in_time
...
def classify_branches(flow, junc_pts, pt_arr):
    inlets = []
    outlets = []  # initialize inlet and outlet list
...
def get_inlet_outlet_pairs(num_inlets, num_outlets):
    ...
    return (
        np.asarray(inlet_list, dtype=np.int32),
        np.asarray(outlet_list, dtype=np.int32),
    )
...
def get_outlet_pairs(num_outlets):
    ...
    for outlet1 in range(num_outlets):
        for outlet2 in range(num_outlets):
            outlet_list1.append(outlet1)
            outlet_list2.append(outlet2)
    return (
        np.asarray(outlet_list1, dtype=np.int32),
        np.asarray(outlet_list2, dtype=np.int32),
    )
...
def get_angle_diff(angle1, angle2):
    try:
        ...
        angle_diff = np.arccos(angle1.T @ angle2).reshape(
            -1,
        )
    except Exception:
        pdb.set_trace()
...
def process_soln(flow_in_time, pressure_in_time, times):
    ...
    times = times[time_sort]  # sort timestep array
    ...
    pressure_in_time_aug, pressure_in_time_aug_der, pressure_in_time_aug_der2 = augment_time(
        pressure_in_time, times, aug_factor
    )
    ...
    return (
        pressure_in_time_aug,
        pressure_in_time_aug_der,
        pressure_in_time_aug_der2,
        flow_in_time_aug,
        flow_in_time_aug_der,
        flow_in_time_aug_der2,
        num_time_steps_model,
    )
...
def verify_bifurcation(flow, area, pressure, junc_pts, pt_arr, verbose=False):
    ...
    inlet_pts = get_inds(arr=pt_arr, vals=inlets)
    outlet_pts = get_inds(arr=pt_arr, vals=outlets)

    if len(inlets) != 1 or len(outlets) != 2:
        if verbose:
            print("Not a Bifurcation")
        bif = False

    elif area[inlet_pts] < area[outlet_pts][0] or area[inlet_pts] < area[outlet_pts][1]:
        if verbose:
            print("Inlet smaller than outlet.")
        bif = False
...
def get_soln_at_time(
    ...
    flow_hist1, flow_hist2, flow_hist3 = get_flow_hist(
        flow_in_time_aug, time_index, num_time_steps_model
    )  # get flow history
