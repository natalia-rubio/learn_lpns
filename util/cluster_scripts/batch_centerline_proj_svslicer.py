...
def call_svslicer(combined_vtu_path, centerline_path, output_path, num_threads=None):
    ...
    try:
        ...
        try:
            result = subprocess.run(cmd, env=env, capture_output=True, text=True, timeout=SVSLICER_TIMEOUT_SECONDS)
...
def project_results_python_fallback(
    geo_dir, sim_dir, centerline_path, num_procs, output_path, time_files, num_workers=None
):
    """
...
def process_geometry(
    ...
    try:
        ...
        if use_python_only or os.getenv("SVSLICER_USE_PYTHON_ONLY", "").lower() in (
            "1",
            "true",
            "yes",
        ):
            ...
            return project_results_python_fallback(
                geo_dir, sim_dir, centerline_path, num_procs, output_path, time_files
            )
        ...
        print(
            f"  Processing all timesteps failed (svSlicer returned: {svslicer_success}, output exists: {output_exists})"
        )
        ...
        for batch_start in range(0, len(time_files), batch_size):
            ...
            print(
                f"  Processing batch {batch_start // batch_size + 1} (timesteps {batch_times[0]} to {batch_times[-1]})..."
            )
