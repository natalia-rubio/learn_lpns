...
def identify_segment_boundary_points(branch_id, num_points_near_boundary=5):
    """
    Identify points that are within num_points_near_boundary points of segment boundaries.
    Segment boundaries are identified where BranchId changes.
    Args:
        branch_id: Array of branch IDs for each point
        num_points_near_boundary: Number of points on each side of boundary to include (default: 5)
    Returns:
        Boolean array indicating which points to process (True = within boundary region)
    """
    num_points = len(branch_id)
    include_mask = np.zeros(num_points, dtype=bool)
    
    if num_points == 0:
        return include_mask
    
    # Find segment boundaries (where BranchId changes)
    # Boundaries occur at transitions between different branch IDs
    boundary_indices = []
    
    for i in range(1, num_points):
        if branch_id[i] != branch_id[i-1]:
            # This is a boundary point
            boundary_indices.append(i)
            # Also mark the previous point as a boundary
            boundary_indices.append(i-1)
    
    # Also include first and last points of each segment
    # Find unique branch IDs and their ranges
    ...
    for branch in unique_branches:
        ...
        if len(branch_indices) > 0:
            ...
            boundary_indices.append(branch_indices[0])
            # Last point of segment
            boundary_indices.append(branch_indices[-1])
    
    # Remove duplicates and sort
    boundary_indices = sorted(set(boundary_indices))
    
    # Mark points within num_points_near_boundary of each boundary
    for boundary_idx in boundary_indices:
        # Include points before boundary
        start_idx = max(0, boundary_idx - num_points_near_boundary)
        end_idx = min(num_points, boundary_idx + num_points_near_boundary + 1)
...
def call_svslicer(combined_vtu_path, centerline_path, output_path, num_threads=None):
    ...
    try:
        ...
        try:
            ...
            print("  Error: svSlicer timed out after 1 hour")
            return False
...
def project_results_python_fallback(geo_dir, sim_dir, centerline_path, num_procs, output_path, time_files, num_workers=None, num_points_near_boundary=5):
    """
    Optimized Python-based projection with caching and parallelization.
    Only processes points within num_points_near_boundary points of segment boundaries.
    Args:
    ...
    try:
        print("  Using optimized Python-based projection (segment boundaries only)...")
        ...
        # Identify points near segment boundaries
        include_mask = identify_segment_boundary_points(branch_id, num_points_near_boundary)
        num_points_to_process = np.sum(include_mask)
        num_total_points = len(branch_id)
        
        print(f"  Filtering points: {num_points_to_process}/{num_total_points} points within {num_points_near_boundary} points of segment boundaries")
        ...
        # Prepare data for parallel processing - only for points near boundaries
        ids = vtk.vtkIdList()
        num_points = reader_1d.GetNumberOfPoints()
        
        # Pre-compute point data - only for included points
        point_data = []
        for i in range(num_points):
            if not include_mask[i]:
                continue  # Skip points not near boundaries
        ...
        print(f"  Extracting solution at {num_points_to_process} points for {len(times)} timesteps")
...
def process_geometry(geo_dir, sim_dir, centerline_path, num_procs, output_path, num_threads=None, batch_size=10, use_python_only=False, num_points_near_boundary=5):
    """
    Process a single geometry: combine VTU files and call svSlicer.
    If svSlicer crashes with many timesteps, process in batches.
    This version only extracts solutions for points within num_points_near_boundary of segment boundaries.
    Args:
        geo_dir: Geometry directory
        sim_dir: Simulation results directory
        centerline_path: Path to centerline VTP file
    ...
    try:
        ...
        # Note: svSlicer doesn't support filtering, so we always use Python method for segment boundaries
        print("  Using Python-based projection (segment boundaries only, svSlicer skipped)")
        return project_results_python_fallback(geo_dir, sim_dir, centerline_path, num_procs, output_path, time_files, num_workers=None, num_points_near_boundary=num_points_near_boundary)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python batch_centerline_proj_svslicer_segment_boundaries.py <set_name> [num_procs] [num_points_near_boundary]")
        print("  set_name: Name of the set to process (required)")
        print("  num_procs: Number of processors (optional, default: 48)")
        print("  num_points_near_boundary: Number of points on each side of boundary to include (optional, default: 5)")
        print("")
        print("Note: This version only extracts solutions for points near segment boundaries.")
        sys.exit(1)
    
    set_name = sys.argv[1]
    num_procs = sys.argv[2] if len(sys.argv) > 2 else "48"
    num_points_near_boundary = int(sys.argv[3]) if len(sys.argv) > 3 else 5
    ...
    print(f"Extracting solutions for points within {num_points_near_boundary} points of segment boundaries")
    print()
    ...
    for geo_name in geo_names:
        ...
        output_path = os.path.join(output_geo_dir, f"unsteady_soln_{num_points_near_boundary}.vtp")
        if os.path.exists(output_path):
            print(f"  Skipping {geo_name}: Output already exists")
            continue
        
        print(f"  Processing {geo_name}...")
        if process_geometry(geo_dir, sim_dir, centerline_path, num_procs, output_path, num_threads=None, batch_size=10, use_python_only=True, num_points_near_boundary=num_points_near_boundary):
            total_processed += 1
        else:
            total_failed += 1
        print()
